...
public interface TrackRepository extends JpaRepository<Track, UUID> {
    ...
    int updateStatusByIds(@Param("ids") List<UUID> ids, @Param("status") LifecycleStatus status);

    @Modifying
    @Query("UPDATE Track t SET t.status = :status WHERE t.albumId = :albumId")
    int updateStatusForAllInAlbum(@Param("albumId") UUID albumId, @Param("status") LifecycleStatus status);
    ...
    long countByAlbumIdAndStatus(@Param("albumId") UUID albumId, @Param("status") LifecycleStatus status);
    ...
    List<Track> findAllReadyForPublishing(@Param("status") LifecycleStatus status, @Param("now") Instant now);
}
