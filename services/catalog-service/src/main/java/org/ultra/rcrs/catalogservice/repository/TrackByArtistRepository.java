...
public interface TrackByArtistRepository extends ReactiveCassandraRepository<TrackByArtist, TrackByArtist.TrackByArtistKey> {
    ...
    Flux<TrackByArtist> findAll(UUID artistId, List<EntityStatus> trackStatuses, List<ArtistRole> artistRoles, Sort.Direction direction);

    default Flux<TrackByArtist> findAllTracks(UUID artistId, List<EntityStatus> trackStatuses, List<ArtistRole> artistRoles, Sort.Direction direction) {
